...
t−1
​
  をクエリとして、外部ストア（Artifact Store）から関連情報を検索する 。
3. Qualify (A 
t
+
​
...
t
​
 ) をフィルタリングし、現在の意思決定に真に必要な情報 (A 
t
+
​
 ) だけを選別する 。
4. Compress & Commit (CCS 
t
​
 ):
    ◦ 入力: x 
...
​
 （選別された外部情報）
    ◦ 処理: CCM（LLM）を使用し、上記の情報を統合して新しい CCS 
t
​
  を生成する。
...
エージェントのシステムプロンプトには、常に最新の CCS 
t
​
  の内容（JSONまたはフォーマットされたテキスト）が挿入されるようにする。
6. 実装上の注意点
...
--------------------------------------------------------------------------------
この仕様書をAIアシスタントに提示し、「この仕様に基づいて acc_agent.py と schemas.py のコードを生成してください」と指示することで、論文に忠実な実装が可能になります。